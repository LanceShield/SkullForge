...
public class itemPaintScraper extends Item {

	public itemPaintScraper(int par1) {
		super(par1);
	...
	{
	this.itemIcon = ir.registerIcon("skullforge:MyPaintScraper");
	}
	@Override
        ...
        else 
        {
	        ...
	        if (blockid == Block.skull.blockID) {
                ...
                if (skullentity != null && skullentity instanceof TileEntitySkull)
                {
                    ...
                    // Check to see that it's the proper head skull
                    if (((TileEntitySkull)skullentity).getSkullType() == 3) { 
	                    ((TileEntitySkull)skullentity).setSkullType(3, ""); // Line that updates the head's NBT
						        ...
						        for (loopPlayers = 0; loopPlayers < par3World.playerEntities.size(); ++loopPlayers)
						        {
						            ...
						            if (playerinloop instanceof EntityPlayerMP) {
						            	...
						            	if (p1.worldObj == p2.worldObj) {
						            		// This sends out the update to everybody on the same dimension
